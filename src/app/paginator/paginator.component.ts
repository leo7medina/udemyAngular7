...
export class PaginatorComponent implements OnInit, OnChanges {
  ...
  private initPaginator(): void{
    this.desde = Math.min(Math.max(this.paginator.number-4),this.paginator.totalPages-5);
    this.hasta = Math.max(Math.max(this.paginator.totalPages, this.paginator.number+4), 6);
